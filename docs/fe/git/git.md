...
这里的版本号是指错误记录的前一个版本号，也就是最后一个正确的版本号。例如，11~20 的版本是有问题的，需要回退，那么 `git reset` 指定的版本号就是 10 号。

所以，找到正确的回退版本号非常重要，特别是回退 master 分支的代码时，建议先另外拉一个分支来试试手，没有问题再操作 master 分支。

硬性回退之后，由于本地的 git 记录相较于远程仓库是滞后的，因此无法推送到远程仓库。需要强制覆盖推送 `git push -f`。
