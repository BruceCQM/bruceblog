...
小程序受限于小程序平台的能力，很多H5渠道的优化手段无法实施或者无效。小程序的性能优化主要依赖小程序平台提供的能力。

### 1、异步分包

异步分包可以理解为主包的扩充，而且仅在使用时异步加载，使用该机制可以有效减小小程序分包、总包的体积大小，减少小程序加载耗时，提升页面渲染速度。

目前微信、支付宝、抖音小程序平台均支持该能力，内部框架也完成对小程序异步分包配置的支持（分包异步化），壳工程根目录 config.json 文件中 optimizeAsyncPackage 属性即为异步分包配置，开发者仅需关注将哪些组件放入异步分包，脚手架会完成剩余工作。

需要注意的是，异步分包的数量无上限，对于放入异步分包中的组件，如果组件之间没有互相依赖，可以考虑多拆异步分包（异步分包组件在抖音平台不能跨异步分包引用，微信可以，微信平台即使组件之间有依赖也可放置在不同的异步分包）。如果异步分包单包体积过大也会影响优化效果。

### 2、组件样式去冗余

微信官方文档[组件样式隔离](https://developers.weixin.qq.com/miniprogram/dev/framework/custom-component/wxml-wxss.html#%E7%BB%84%E4%BB%B6%E6%A0%B7%E5%BC%8F%E9%9A%94%E7%A6%BB)

目前小程序中组件的样式在组件本身 js 中引用无法生效（微信小程序机制问题），导致所有组件样式要么在页面中引入，要么在主包app.scss中引入。

由于 H5 中不存在组件样式隔离机制问题，所以在开发中会存在很多组件内部 jsx 引用了自身样式，后续为了兼容小程序，在使用该组件的页面又引用组件的样式，这样会导致小程序中同一个组件存在 2 份样式，分包体积增大。

就算组件样式仅在页面中引用，但是很多业务组件会在多个页面中使用，每个页面都引用一次组件样式，也会导致分包中该组件样式其实被 N 个页面重复引入。

解决方案：

组件添加 styleIsolation 配置，并在组件 js 中引用自身样式文件。

不支持样式隔离配置的小程序平台（抖音、快手、百度）样式引用仍保持当前机制。

即在组件、页面中针对渠道判断，如果是不支持的平台则保留页面引用语句去除组件内部引用样式，支持样式隔离配置的小程序平台保留组件内部自身样式引用，去除页面引用组件样式的语句。
...
// 不是抖音、百度、快手小程序平台，就在组件内部引用样式文件
// 其实就只剩微信小程序了
if (!['dy', 'bd', 'ks'].includes(process.env.TARO_ENV)) {
  require('./index.scss');
}
...
不支持样式隔离的小程序，如抖音、百度、快手：

```jsx
// myPage.jsx
// 在页面中引入组件的样式
if (['tt', 'swan', 'kwai'].includes(process.env.TARO_ENV)) {
  require('myComponent/index.scss');
}
```
...
软件工程没有银弹，一种优化方案可能适用于大多数项目，但是某些特殊情况下很可能会起反效果。

举个例子：图片转成base64，仅适用于非常小的图片，base64编码后图片会变大1/3，体积变大和请求开销对比，适合icon网站。