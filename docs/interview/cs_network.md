...
[小林coding-TCP三次握手过程是怎样的](https://xiaolincoding.com/network/3_tcp/tcp_interview.html#tcp-%E4%B8%89%E6%AC%A1%E6%8F%A1%E6%89%8B%E8%BF%87%E7%A8%8B%E6%98%AF%E6%80%8E%E6%A0%B7%E7%9A%84){link=card}

[TCP协议三次握手及四次挥手](https://blog.csdn.net/qq_44129924/article/details/114639559){link=static}

## TCP 四次挥手

### 四次挥手主要流程

三次握手建立连接后，客户端和服务端都可以主动发起四次挥手断开连接，下面以客户端主动发起挥手为例说明。

![四次挥手主要流程](./images/cs_network/TCP_four_way_wavehand.png)

- 客户端打算关闭连接，它向服务端发送 FIN 报文，随后进入 FIN_WAIT_1 状态。

- 服务端收到客户端的 FIN 报文后，随即发送 ACK 报文，表示收到了客户端的关闭请求，随后进入 CLOSE_WAIT 状态。

- 客户端收到服务端的 ACK 报文后，知道服务端已经收到了自己发送的 FIN 报文，进入 FIN_WAIT_2 状态。

- 等服务端处理完数据，做好关闭连接的准备后，它也向客户端发送一个 FIN 报文，表示自己已经做好关闭连接的准备，随后进入 LAST_ACK 状态。

- 客户端收到服务端的 FIN 报文，随即发送 ACK 应答报文，进入 TIME_WAIT 状态。

- 服务端收到客户端的 ACK 报文后，进入 CLOSE 状态，至此服务端已经完成连接的关闭。

- 客户端经过 2MSL 时间之后，也进入 CLOSE 状态，至此客户端也完成连接的关闭。

每个方向都需要一次 FIN 和一次 ACK，因此被称为四次挥手。

### 为何挥手需要四次？

在三次握手建立连接的过程中，第二次握手是 SYN+ACK 合并发送的。

而在关闭连接时，当服务端收到客户端的 FIN 报文时，它可能还有部分数据没有发送完成，所以只能先发送 ACK 确认应答，表示自己已经收到客户端的关闭请求。

等数据发送完成之后，服务端再发送 FIN 报文，表示自己数据已经处理完成，做好关闭连接的准备。

服务端的 ACK 和 FIN 没有合并发送，因此需要四次挥手。

### 可以三次挥手吗？

理论上是可以的，服务端可以等到自己处理完数据，CLOSE_WAIT 状态过了之后，把第二次挥手的 ACK 和第三次挥手的 FIN 合并一块发送，这样子就是三次挥手了。

但如果服务端处理数据需要比较长的时间，可能会导致客户端收不到 ACK 应答报文而重发 FIN 报文。如果客户端重发 FIN 报文超过次数上限后，就会断开连接。

在实际情况中，三次挥手也确实是可能比四次挥手更常见，因为开启了 「TCP 延迟确认机制」。

:::danger 什么是 TCP 延迟确认机制？
发送没有携带数据的 ACK 报文的传输效率是很低的，因为它也有 IP 头和 TCP 头，但却没有携带数据。为了解决 ACK 传输效率低的问题，出现了 TCP 延迟确认机制，它的策略如下：

- 当有响应数据要发送时，ACK 会随着响应数据一起立刻发送给对方。

- 当没有响应数据要发送时，ACK 会延迟一段时间，等等看是否有数据可以一起发送。

- 如果在延迟等待发送 ACK 期间，对方第二个数据报文又到达了，会立刻发送 ACK。
:::

作用在四次挥手关闭连接的过程中，服务端在发送 ACK 之前，会延迟一段时间，如果这段时间内服务端处理完数据，做好关闭连接的准备，就会把 ACK+FIN 报文合并一块发送给客户端，变成了三次挥手。

### 为何要等待 2MSL 的时间？

MSL 是 Maximum Segment Lifetime，报文最大生存时间，它是任何报文在网络中存在的最长时间，超过这个事件报文将被丢弃。

- 在传输过程中最后的 ACK 报文可能超时或者丢失了，所以 TIME_WAIT 状态就是⽤来重发可能丢失的 ACK 报⽂，2MSL 的时长就是允许 ACK 报文至少丢失一次。比如，ACK 在一个 MSL 时间内丢失了，服务端重发的 FIN 报文会在第二个 MSL 时间内到达。
