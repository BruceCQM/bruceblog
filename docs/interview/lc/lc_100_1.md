...
prefix 表示从 0 到当前下标 i 的前缀和：prefix = nums[0] + nums[1] + ... + nums[i]

任意一段子数组 [j+1, j+2, ..., i] 的和可以写成：子数组和 = prefix[i] - prefix[j]

我们要的是：子数组和 = k，即：prefix[i] - prefix[j] = k

移项得：prefix[j] = prefix[i] - k

也就是说：只要在位置 i 之前，出现过前缀和等于 prefix[i] - k 的某个位置 j，那么从 **j+1 到 i 这一段子数组的和就是 k**。

2. `map` 在存什么

- key：某个前缀和的值（即某个 prefix[j]）
- value：这个前缀和在当前位置 i 之前已经出现过的次数

所以 map 表示的是：「到当前为止，每个前缀和值出现了几次」。

3. `if (map.has(prefix - k)) { count += map.get(prefix - k); }` 的含义是：

在当前位置 i，看一下之前有多少个前缀和等于 prefix - k，就有多少个以 i 结尾、和为 k 的子数组，把这些个数全部加到总答案里。
...
### 240. [搜索旋转排序数组 II](https://leetcode.cn/problems/search-a-2d-matrix-ii/)
