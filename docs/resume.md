...
详细讲讲DDD的核心概念有哪些？
拿一个具体的模块来讲，是怎么把它用DDD的，因为DDD一般出现在后端。
DDD，前端这边用法一般就是，领域分层、聚合，分组件、怎么UI渲染。聚合无非就是各种数据的逻辑处理。然后一些保证多端。
编译小程序和RN的桥接，和原生的有什么区别？
RN是什么？原生的APP这一块有了解过吗
React18的并发渲染和之前的同步渲染有什么本质区别？
React的渲染机制是怎么样的？
useState是同步还是异步的？useState更新后的状态，能否立刻拿到最新的状态？
项目中避免不了不必要的重新渲染，可以怎么优化？—— useMemo、useCallback、状态提升状态下沉、对应用的上下文拆分多个组件，对架构层面做一个改造